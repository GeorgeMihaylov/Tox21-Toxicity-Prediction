...
def main() -> int:
    ...
    out_path = reports_dir / "scaffold_cv_threshold_metrics.csv"
    ...
    plt.ylabel("Count")
    plt.title("Threshold distribution (selected on validation)")
    plt.tight_layout()
    plt.savefig(fig_dir / "threshold_distribution_scaffold_cv.png", dpi=300)
    plt.close()

    plt.figure(figsize=(7, 4))
    plt.boxplot([out["roc_auc"].values, out["average_precision"].values, out["f1"].values], labels=["ROC-AUC", "AP", "F1"])
    plt.title("Metrics distribution across scaffold folds")
    plt.tight_layout()
    plt.savefig(fig_dir / "metrics_boxplot_scaffold_cv.png", dpi=300)
    plt.close()
